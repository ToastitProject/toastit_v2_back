...
class TrendCocktailResponseTest {
    ...
    void fromNaverResponse() {
        ...
        // 메서드 호출
        TrendCocktailDTO result = TrendCocktailResponse.fromNaverResponse(response);
        ...
        // 첫 번째 결과 검증
        TrendCocktailDTO.Result firstResult = result.getResults().get(0);
        ...
        // 첫 번째 데이터 검증
        TrendCocktailDTO.Data firstData = firstResult.getData().get(0);
        assertEquals("2024-10-01", firstData.getPeriod());
        assertEquals(100, firstData.getRatio());

        // 두 번째 결과 검증
        TrendCocktailDTO.Result secondResult = result.getResults().get(1);
        ...
        // 두 번째 데이터 검증
        TrendCocktailDTO.Data secondData = secondResult.getData().get(0);
